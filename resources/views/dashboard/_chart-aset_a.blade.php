<div class="col-md-6">
	<div class="card card-custom card-stretch gutter-b chart-pelaporan1-wrapper">
		<div class="card-header h-auto py-3">
			<div class="card-title">
				<h3 class="card-label">
					<span class="d-block text-dark font-weight-bolder">{{ __('Tanah') }}</span>
				</h3>
			</div>
			<div class="card-toolbar" style="max-width: 500px;">
				<form id="filter-chart-pelaporan1"
					action="{{ route($routes.'.chartAsetKIBA') }}"
					class="form-inline"
					role="form">
...
@push('scripts')
	<script>
		$(function() {
		    iniFilterchartTermine();
		    drawchartTermine();
		});

		var iniFilterchartTermine = function () {
		...
		var drawchartTermine = function () {
			var filter = $('#filter-chart-pelaporan1');
		...
		var renderchartTermine = function (options = {}) {
			var element = document.getElementById('chart-pelaporan1');
