...
class KIBDController extends Controller
{
    ...
    public function index(){
        $records = Aset::with('coad')->where('type','KIB D')
        ->get();
        $data = [];
        ...
        foreach ($records as $record) {
            ...
            $data[] = [
                ...
                'nama_kontruksi' => $record->usulans? $record->usulans->trans->vendors->name : '-',
                'panjang' => $record->long_JJR ? $record->long_JJR : '-',
                'lebar' => $record->width_JJR ? $record->width_JJR : '-',
    ...
    public function detail(Request $request){
        $record = Aset::with('coad')->where('type','KIB D')->find($request->id);
        if ($record){
            $user = strip_tags($record->createsByRaw());
            $user = trim($user);
