...
class KIBCController extends Controller
{
    ...
    public function index(){
        $records = Aset::with('coad')->where('type','KIB C')
        ->get();
        ...
        foreach ($records as $record) {
            ...
            $data[] = [
                ...
                'luas_lantai' => $record->wide ? number_format($record->wide, 0, ',', ',') : '-',
                'luas_bangunan' => $record->wide_bld ? number_format($record->wide_bld, 0, ',', ',') : '-',
    ...
    public function detail(Request $request){
        $record = Aset::with('coad')->where('type','KIB C')->find($request->id);
        if ($record){
            $user = strip_tags($record->createsByRaw());
            $user = trim($user);
            ...
            $data[] = [
                ...
                'masa_manfaat' => $record->useful ? $record->useful : '-',
                'akumulasi' => $record->accumulated_depreciation ? number_format($record->accumulated_depreciation, 0, ',', ',') : '0',
                'status' => $record->status ? ($record->status == 'actives' ? '<span class="badge bg-success text-white">'.ucfirst('active').'</span>' : ($record->status == 'notactive' ? '<span class="badge bg-danger text-white">'.ucfirst($record->status).'</span>' : ($record->status == 'in repair' ? '<span class="badge bg-warning text-white">'.ucfirst($record->status).'</span>' : ($record->status == 'in deletion' ? '<span class="badge bg-warning text-white">'.ucfirst($record->status).'</span>' : '<span class="badge bg-light">'.ucfirst($record->status).'</span>')))) : '-',
                'tanah_id' => $record->tanah_id ? $record->tanah_id : '-',
                'kondisi' => $record->condition ? ($record->condition == 'baik' ? ucfirst($record->condition) : ($record->condition == 'rusak berat' ? ucfirst($record->condition) : ucfirst($record->condition))) : '-',
                // 'kondisi' => $record->condition ? ($record->condition == 'baik' ? '<span class="badge bg-success text-white">'.ucwords($record->condition).'</span>' : ($record->condition == 'rusak berat' ? '<span class="badge bg-danger text-white">'.ucwords($record->condition).'</span>' : '<span class="badge bg-warning text-white">'.ucwords($record->condition).'</span>')) : '-',
                'keterangan' => $record->description ? $record->description : '-',
