...
class CoaKontruksiBangunanController extends Controller
{
    //
    protected $module = 'master_coa_kontruksi_pembangunan';
    protected $routes = 'master.coa.kontruksi-pembangunan';
    ...
    public function __construct()
    {
        $this->prepare(
            [
                ...
                'title' => 'Chart of Accounts Kontruksi dalam Pembangunan',
                'breadcrumb' => [
                    'Data Master' => rut($this->routes . '.index'),
                    'Chart Of Accouts' => rut($this->routes . '.index'),
                    'COA Kontruksi dalam Pembangunan' => rut($this->routes . '.index'),
                ]
            ]
        );
    }
    ...
    {
        $user = auth()->user();
        $records = Coa::grid()->where('tipe_akun','=','KIB F')->filters()->dtGet();
