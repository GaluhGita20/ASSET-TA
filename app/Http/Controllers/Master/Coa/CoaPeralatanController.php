...
class CoaPeralatanController extends Controller
{
    //
    protected $module = 'master_coa_peralatan';
    protected $routes = 'master.coa.peralatan-mesin';
    ...
    public function __construct()
    {
        $this->prepare(
            [
                ...
                'title' => 'Chart of Accounts Peralatan dan Mesin',
                'breadcrumb' => [
                    'Data Master' => rut($this->routes . '.index'),
                    'Chart Of Accouts' => rut($this->routes . '.index'),
                    'COA Peralatan & Mesin' => rut($this->routes . '.index'),
                ]
            ]
        );
    }
    ...
    {
        $user = auth()->user();
        $records = Coa::grid()->where('tipe_akun','KIB B')->filters()->dtGet();
