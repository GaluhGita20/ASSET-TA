...
class PerbaikanAsetController extends Controller
{
    ...
    protected $routes = 'pengajuan.perbaikan-aset';
    protected $views = 'pengajuan.perbaikan-aset';
        ...
        $this->prepare([
            'tableStruct' => [
                'datatable_1' => [
                  ...
                  //  $this->makeColumn('name:tanggal_pengajuan|label:Tanggal Pengajuan|className:text-center|width:300px'),
    ...
    {
        $user = auth()->user();
        $records = Perbaikan::grid()->filters()->dtGet();
        ...
        return DataTables::of($records)
            ...
            ->addColumn('hasil_perbaikan', function ($record) use ($user) {
                if($record->repair_results == 'SELESAI'){
                    return $record->repair_results ? '<span class="badge bg-success text-white">'.ucfirst($record->repair_results).'</span>' : '-';
                }elseif($record->repair_results == 'BELUM SELESAI'){
                    return $record->repair_results ? '<span class="badge bg-warning text-white">'.ucfirst($record->repair_results).'</span>' : '-'; 
                }else{
                    return $record->repair_results ? '<span class="badge bg-danger text-white">'.ucfirst($record->repair_results).'</span>' : '-';
                }
            })
            ...
            ->addColumn('action', function ($record) use ($user) {
                ...
                if (auth()->user()->checkPerms('perbaikan-aset.edit') && $record->status =='draft' ) {
                    $actions[] = [
                        'type' => 'delete',
                        'id' => $record->id,
                        'method'=>'post',
                ...
                if(auth()->user()->hasRole('Sarpras') && $record->status =='approved' && $record->repair_results == 'BELUM SELESAI' && auth()->user()->checkPerms('perbaikan-aset.edit')){
                    $actions[] = [
                        'type' => 'edit',
                        'label' => 'Update Repair',
