...
class PerencanaanAsetPelayananController extends Controller
{
    protected $module = 'perencanaan-aset-pelayanan';
    protected $routes = 'pengajuan.perencanaan-aset-pelayanan';
    protected $views = 'pengajuan.perencanaan-aset';
    protected $perms = 'perencanaan-aset-pelayanan';
    ...
    public function grid()
    {
        ...
        return DataTables::of($records)
            ...
            ->addColumn('action', function ($record) use ($user) {
                ...
                if(auth()->user()->position->location->level != 'subdepartment'){ //untuk level departemen dan bod

                    $approval1= $record->whereHas('approvals', function ($q) use ($record) {
                        $q->where('target_id',$record->id)->where('status','!=','approved')->where('order',1)->where('role_id',5);
                    })->count();

                    if($user->position->location->level == 'bod'){
                    ...
                    }elseif(auth()->user()->position->location->level == 'department' && auth()->user()->position->location->name !='Bidang Penunjang Medik dan Non Medik' && $approval1 == 1 ){
                        if ($record->checkAction('approval', $this->perms)) {
                            $actions[] = [
                                'type' => 'approval',
                                'label' => 'Approval',
                    ...
                    }elseif(auth()->user()->position->location->level == 'department' && auth()->user()->position->location->name =='Bidang Penunjang Medik dan Non Medik' && $approval1 == 0 ){
                        if ($record->checkAction('approval', $this->perms)) {
                            $actions[] = [
                                'type' => 'approval',
                                'label' => 'Approval',
                    ...
                    // level bod (direksi)          ==> aproval tahap  4
                    // level perencanaan            ==> approval tahap 3
                    // level departemen penunjang   ==> approval tahap 2
                    // level departemen umum        ==> approval tahap 1
                ...
                }elseif($user->position->location->id == 13){ // untuk level keuangan
                    if ($record->checkAction('approval', $this->perms)) {
                        $actions[] = [
                            'type' => 'approval',
                            'label' => 'Approval',
    ...
    {        
        ...
        return DataTables::of($records)
            ...
            ->rawColumns(['qty_agree','action', 'action_show','status','action_approval' ,'updated_by','created_by'])
            ->make(true);
    }
        ...
        $this->prepare([
            'tableStruct' => [
                'datatable_1' => [
                    ...
                    $this->makeColumn('name:qty_agree|label:Disetujui|className:text-center|width:100px'),
                    ...
                    $this->makeColumn('name:status|width:100px'),
    ...
    public function print(Perencanaan $record, $title = '')
    {
        ...
        $view1 = view($this->views.'.cetak1',compact('record','title','gambar_logo_1','gambar_logo_2'))->render();
        $view2 = view($this->views.'.cetakDetail',compact('detail','record','title','gambar_logo_1','gambar_logo_2'))->render();
        $html = $view1.'<div style="page-break-after:always;"></div>'.$view2;
